 ...
 */
public class GolombCompressedSet2 implements Filter {

    private final long seed;
    private final BitBufferDirect buff;
    private final int golombShift;
    private final int bufferSize;
    private final int bucketCount;
    private final int fingerprintMask;
    private final MonotoneList2 start;
    private final int startBuckets;

    public static GolombCompressedSet2 construct(long[] keys, int setting, long seed) {
        return new GolombCompressedSet2(keys, keys.length, setting, seed);
    }

    GolombCompressedSet2(long[] keys, int len, int fingerprintBits, long seed) {
        ...
        BitBufferDirect buckets = new BitBufferDirect(10L * fingerprintBits * len);
        ...
        buff = new BitBufferDirect(10L *  fingerprintBits * len);
        buff.writeEliasDelta(len + 1);
        start = MonotoneList2.generate(startList, buff);
