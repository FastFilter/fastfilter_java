 ...
 */
public class Cuckoo8 implements MutableFilter {
    ...
    public static Cuckoo8 construct(long[] keys, LongSupplier seedingStrategy) {
        int len = keys.length;
        while (true) {
            try {
                Cuckoo8 f = new Cuckoo8((int) (len / 0.95), seedingStrategy.getAsLong());
                for (long k : keys) {
                    f.add(k);
                }
                return f;
