 ...
 */
public class MonotoneListTest2 {
    ...
    public static void main(String... args) {
        ...
        testPerformance();
        testBestSize(100, 8);
        new MonotoneListTest2().testSaving();
    }
    ...
    private static void testBestSize(int bucketSize, int size) {
        ...
        String best = "";
        BitBufferDirect buffer = new BitBufferDirect(100 * bucketCount);
        MonotoneList2.generate(posList, buffer);
        assertEquals(MonotoneList2.getSize(posList), buffer.position());
            ...
            for (int shift2 = 1; shift2 <= shift1; shift2++) {
                for (int factor1 = 4; factor1 < 256; factor1 *= 2) {
                    for (int factor2 = 2; factor2 <= factor1; factor2 *= 2) {
                        buffer = new BitBufferDirect(100 * bucketCount);
                        // MultiStageMonotoneList2.SHIFT1 = shift1;
                        // MultiStageMonotoneList2.SHIFT2 = shift2;
                        // MultiStageMonotoneList2.FACTOR1 = factor1;
                        // MultiStageMonotoneList2.FACTOR2 = factor2;
    ...
    private static void test(int bucketSize, int size) {
        ...
        BitBufferDirect buffer = new BitBufferDirect(1000 + 100 * size);
        MonotoneList2 list = MonotoneList2.generate(posList2, buffer);
        assertEquals(MonotoneList2.getSize(posList2), buffer.position());
        ...
        buffer.seek(0);
        MonotoneList2 list2 = MonotoneList2.load(buffer);
